...
/* 16 Key USB Emulator.
   ...
   This code turns your ProMicro, with an ATmega32U4,
   into an HID USB Keypad.
...
#define _Arduino_UNO  0          // NOT clone board, on clone there NO USB IC that can be programmet to USB Simulator.
#define _Arduino_Mini 0          // NOT clone board, on clone there NO USB IC that can be programmet to USB Simulator.
#define _Arduino_Nano 1          // NOT clone board, on clone there NO USB IC that can be programmet to USB Simulator.
#define _SparkFun_Pro_Micro 0    // Only Testet with Sparkfun and Clone board with 32U4 Processor sometime call Leornardo Board but PinOut are for Sparkfun ProMicro board.
//------------------------------------
...
#define _DEBUG 1   // Uses Debug or Not  Set to 1 if Debug and 0 if No Debug
...
/* sendKeyPress(int key): This function sends a single key over USB
  ...
  -----------------------------------------
   | Enter   | Return  | ESC  | BackSpace |
   | Up      | Down    | Left | Right     |
   | Page-up | Page-dn | Home | End       |
   | Insert  | Delete  | F1   | F10       |
   ----------------------------------------
*/
#if (_DEBUG==1)
...
void sendKeyPress(int key)
{
  switch (key)
  {
    ...
    case 2:  // 0x002
      ...
      break;
    case 4:  // 0x004
      Keyboard.write(KEY_ESC);  // Sends a keyboard Enter-tast
      break;
    case 8:  // 0x008
      Keyboard.write(KEY_BACKSPACE);  // Sends a keyboard Enter-tast
      break;
    case 16:  // 0x010
      Keyboard.write(KEY_UP_ARROW);  // Sends a keyboard Enter-tast
      break;
    case 32:  // 0x020
      Keyboard.write(KEY_DOWN_ARROW);  // Sends a keyboard Enter-tast
      break;
    case 64:  // 0x040
      Keyboard.write(KEY_LEFT_ARROW);  // Sends a keyboard Enter-tast
      break;
    case 128:  // 0x080
      Keyboard.write(KEY_RIGHT_ARROW);  // Sends a keyboard Enter-tast
      break;
    case 256:  // 0x100
      Keyboard.write(KEY_PAGE_UP);  // Sends a keyboard Enter-tast
      break;
    case 512:  // 0x200
      Keyboard.write(KEY_PAGE_DOWN);  // Sends a keyboard Enter-tast
      break;
    case 1024:  // 0x400
      Keyboard.write(KEY_HOME);  // Sends a keyboard Enter-tast
      break;
    case 2048:  // 0x800
      Keyboard.write(KEY_END); // Sends a keyboard Enter-tast
      break;
    case 4096:  // 0x1000
      Keyboard.write(KEY_DELETE);  // Sends a keyboard Enter-tast
      break;
    case 8192:  // 0x2000
      Keyboard.write(KEY_INSERT);  // Sends a keyboard Enter-tast
      break;
    case 16384: // 0x4000
      Keyboard.write(KEY_F10);  // Sends a keyboard Enter-tast
      break;
    case 32768:  // 0x8000
      Keyboard.write(KEY_F1); // Sends a keyboard Enter-tast
      break;
  }
}
#endif
